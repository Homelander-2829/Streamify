...
const OnboardingPage = () => {
  ...
  const { mutate: onboardingMutation, isPending } = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      // Force refetch the authUser query instead of manual cache update
      await queryClient.refetchQueries({ queryKey: ["authUser"] });
      
      // Check the updated data
      const updatedUser = queryClient.getQueryData(["authUser"]);
      console.log("Refetched user data:", updatedUser);
    },
    onError: (error) => {
  ...
  const handleRandomAvatar = () => {
    const randomSeed = Math.floor(Math.random() * 1000000); // or use UUID/userID/email
